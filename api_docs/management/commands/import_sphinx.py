...
from api_docs.importers.sphinx import SphinxImporter
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option(
            ...
            "--inventory",
            dest="inv",
            help="Spinx objects.inv import documentation from",
        ),
        make_option(
            "-s",
            "--sections",
    ...
    def handle(self, *args, **options):
        ...
        importer = SphinxImporter(topic, language, version, section, options)
        importer.run()
